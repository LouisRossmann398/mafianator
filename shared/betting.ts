import { RETROACTIVE_TIPS } from "./leagues";
import type { Bet, LeagueKey, LeagueMatch } from "./types";
import { calcPoints } from "./scoring";

export function groupMatchesByRound(
  matches: LeagueMatch[],
  leagueKey: LeagueKey,
): Map<number, LeagueMatch[]> {
  const byRound = new Map<number, LeagueMatch[]>();
  for (const m of matches) {
    if (m.leagueKey !== leagueKey) continue;
...
export function sortedRounds(matches: LeagueMatch[], leagueKey: LeagueKey): number[] {
  return [...groupMatchesByRound(matches, leagueKey).keys()].sort((a, b) => a - b);
}

export function defaultRoundIndex(
  rounds: number[],
  matches: LeagueMatch[],
  leagueKey: LeagueKey,
): number {
  if (rounds.length === 0) return 0;
  if (RETROACTIVE_TIPS) return rounds.length - 1;
  const now = Date.now();
...
export function isMatchOpenForTips(match: LeagueMatch): boolean {
  if (!match.tippable) return false;
  if (RETROACTIVE_TIPS) return true;
  if (match.result) return false;
  return new Date(match.kickoff).getTime() > Date.now();
}

export function isMatchFinished(match: LeagueMatch): boolean {
  return !!match.result;
}

export function pointsForBet(bet: Bet | undefined, match: LeagueMatch): number | undefined {
