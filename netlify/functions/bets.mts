import { RETROACTIVE_TIPS } from "@shared/leagues";
import type { Bet } from "@shared/types";
import { requireAuth } from "./_lib/auth.ts";
import { stores$ } from "./_lib/blobs.ts";
import { getLeagueMatchById } from "./_lib/league-match-store.ts";
...
export default async (req: Request): Promise<Response> => {
  ...
  if (req.method === "DELETE") {
    ...
    const match = await getLeagueMatchById(matchId);
    if (
      match &&
      !RETROACTIVE_TIPS &&
...
async function saveBet(
  ...
  const match = await getLeagueMatchById(payload.matchId);
  if (!match) return { message: "Spiel nicht gefunden" };
  if (!match.tippable || match.involvesSvp) {
    return { message: "Auf SVP-Spiele kann nicht getippt werden" };
  }
  if (!RETROACTIVE_TIPS && new Date(match.kickoff).getTime() <= Date.now()) {
