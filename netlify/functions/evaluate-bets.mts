...
import { calcPoints } from "@shared/scoring";
import { stores$ } from "./_lib/blobs.ts";
import { loadLeagueMatches } from "./_lib/league-match-store.ts";

export async function runEvaluation(): Promise<{
  evaluated: number;
  reEvaluated: number;
}> {
  const [matches, bets] = await Promise.all([loadLeagueMatches(), stores$.bets().all()]);
