...
import { useLeagueMatches, useBfvSync } from "@/api/league-matches";
import { useMyBets, useSubmitBet, useSubmitBetsBulk } from "@/api/bets";
import { useToast } from "@/components/ui/toast";
import { formatDate, formatTime } from "@/lib/format";
import { cn } from "@/lib/cn";
import type { LeagueKey, LeagueMatch, Bet } from "@shared/types";
import {
  defaultRoundIndex,
  groupMatchesByRound,
  isMatchFinished,
...
import { LEAGUE_LABELS, RETROACTIVE_TIPS, TIP_SEASON_LABEL } from "@shared/leagues";
import { randomRealisticScore } from "@shared/random-scores";

const LEAGUE_TABS: { key: LeagueKey; label: string }[] = [
  { key: "kreisklasse", label: "Kreisklasse" },
  { key: "c-klasse", label: "C-Klasse" },
];
...
export function TippenPage() {
  const { data, isLoading, isFetching, refetch } = useLeagueMatches();
  ...
  const openTippable = tippableMatches.filter((m) => !betFor(m.id));
  const finishedTippable = tippableMatches.filter((m) => !!betFor(m.id));
  ...
  const onKiTipps = async () => {
    if (openTippable.length === 0) {
      toast({ title: "Keine offenen Tipps in diesem Spieltag", variant: "destructive" });
      return;
    }
    ...
    <div className="space-y-4">
      ...
      <Card>
        <CardContent className="p-4 text-xs space-y-2">
          <div className="font-semibold text-sm">Saison {TIP_SEASON_LABEL}</div>
          ...
          {RETROACTIVE_TIPS && (
            <p className="text-muted-foreground">
              Die Saison ist abgeschlossen – du kannst nachträglich noch Tipps abgeben und Punkte
              sammeln.
            </p>
          )}
      ...
      {(isLoading || isFetching) && !matches?.length && (
        <Card>
          <CardContent className="p-5 text-center text-sm text-muted-foreground space-y-2">
            <p>Spiele werden vom BFV geladen…</p>
            <p className="text-xs">Beim ersten Mal kann das ein paar Sekunden dauern.</p>
          </CardContent>
        </Card>
      )}
        ...
        <Card>
          <CardContent className="p-5 text-center text-sm text-muted-foreground space-y-3">
            ...
            {scrapeStatus?.lastError && (
              <p className="text-xs text-destructive/90">BFV: {scrapeStatus.lastError}</p>
            )}
            <div className="flex flex-col gap-2">
              <Button
                ...
                onClick={() => bfvSync.mutate()}
              >
                Spiele vom BFV laden
              </Button>
      ...
      {finishedTippable.length > 0 && (
        <section className="space-y-2">
          <h2 className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
            <History size={12} /> Deine Tipps ({finishedTippable.length})
          </h2>
...
function SvpMatchCard({ match }: { match: LeagueMatch }) {
...
function MatchTeams({ match }: { match: LeagueMatch }) {
  return (
...
function FinishedBetCard({ match, bet }: { match: LeagueMatch; bet?: Bet }) {
